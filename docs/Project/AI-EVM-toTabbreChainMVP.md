...
The Tabbre Project is an ambitious clean energy and blockchain initiative that aims to use floating oceanic solar farms to produce very cheap renewable energy while building a decentralized financial system around that energy economy.  Its core idea is to combine large-scale sustainable energy production with two crypto assets, TABB and BRE, so the project can fund infrastructure, govern operations, price energy sales, and eventually support a global universal basic income.

Tabbre’s roadmap centers on building an initial live TabbreChain, adding wallet, explorer, and public node access, then later introducing BRE monetary governance, broader decentralized finance functions, and energy-linked economic activity.  The project presents itself as a way to accelerate the transition away from fossil fuels while also creating a new global financial framework tied to renewable energy rather than traditional fiat systems.

TABB is Tabbre’s fixed-supply token, designed as the governance, staking, fundraising, and collateral asset for the ecosystem, with value intended to increase over time through scarcity, buybacks, burns, and staking demand.  BRE is the companion token designed to function as a stable-value currency for transaction fees, energy sales, and later broader monetary use, with its value intended to be linked to energy production and managed supply.

TabbreChain is the proof-of-stake blockchain that expresses both TABB and BRE, supports smart contracts, and hosts the governance rules of the Tabbre Foundation DAO.  In this model, TABB is used as staking collateral and for voting, while transaction charges and other usage fees are paid in BRE and validators are rewarded in BRE.

The broader economic idea is that Tabbre uses TABB to raise capital and align governance, while BRE becomes the operational currency for selling renewable energy and potentially funding a global universal basic income.  After the project reaches its “distribution point,” profits are intended to be used to buy and burn TABB, reducing supply while reinforcing TABB’s role as the appreciating asset in the system.
