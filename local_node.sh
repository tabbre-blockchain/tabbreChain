...
add_genesis_funds() {
  local keyname="$1"
  evmd genesis add-genesis-account "$keyname" 1000000000000000000000atabb,1000000000000000000000abre --keyring-backend "$KEYRING" --home "$CHAINDIR"
}
...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
  ...
  # Staking and Governance use TABB
  jq '.app_state["staking"]["params"]["bond_denom"]="atabb"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
  jq '.app_state["gov"]["deposit_params"]["min_deposit"][0]["denom"]="atabb"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
  jq '.app_state["gov"]["params"]["min_deposit"][0]["denom"]="atabb"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
  jq '.app_state["gov"]["params"]["expedited_min_deposit"][0]["denom"]="atabb"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
  
  # Mint is neutralized (0% inflation) to fix TABB supply, but bound to atabb
  jq '.app_state["mint"]["params"]["mint_denom"]="atabb"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
  ...
  # EVM, Gas, and Bank display
  jq '.app_state["evm"]["params"]["evm_denom"]="abre"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"

  # Define both TABB and BRE metadata
  jq '.app_state["bank"]["denom_metadata"]=[{"description":"The staking and governance token for Tabbre.","denom_units":[{"denom":"atabb","exponent":0,"aliases":["attotabb"]},{"denom":"tabb","exponent":18,"aliases":[]}],"base":"atabb","display":"tabb","name":"Tabbre","symbol":"TABB","uri":"","uri_hash":""}, {"description":"The fee and EVM token for Tabbre.","denom_units":[{"denom":"abre","exponent":0,"aliases":["attobre"]},{"denom":"bre","exponent":18,"aliases":[]}],"base":"abre","display":"bre","name":"BRE Gas Token","symbol":"BRE","uri":"","uri_hash":""}]' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
  ...
  jq '.app_state.erc20.dynamic_precompiles=["0x1111111111111111111111111111111111111111"]' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
  jq '.app_state.erc20.token_pairs=[{contract_owner:1,erc20_address:"0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE",denom:"abre",enabled:true},{contract_owner:1,erc20_address:"0x1111111111111111111111111111111111111111",denom:"atabb",enabled:true}]' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
  ...
  evmd genesis add-genesis-account "$VAL_KEY" 100000000000000000000000000atabb,100000000000000000000000000abre --keyring-backend "$KEYRING" --home "$CHAINDIR"
  ...
  # --------- Finalize genesis ---------
  evmd genesis gentx "$VAL_KEY" 1000000000000000000000atabb --fees ${BASEFEE}abre --keyring-backend "$KEYRING" --chain-id "$CHAINID" --home "$CHAINDIR"
